...
function run_DMFT(NSites::Int, U::Float64, μ_in::Float64, β::Float64, fitf::Function; maxit = 20)
    Nk  = 200
    Nν  = 2000
    α   = 0.4
...
function run_tests(NSites, U, μ_in, β; maxit = 50)
    fits = []
    names = []
    i = 1
    global to
    for (transf_name, cf) in zip(transforms_names, transforms_list)
        for (opt_name,opt) in zip(optim_names, optim_list)
            for (dist_name, dist_f) in zip(dist_names, dist_list)
                ...
                p, νnGrid, GImp_i, ΣImp_i, μ, dens, fit_res  = run_DMFT(NSites, U, μ_in, β, fitf; maxit = maxit)
                push!(fits, (p, νnGrid, GImp_i, ΣImp_i, μ, dens, fit_res))
...
names,fits = run_tests(NSites, U, μ, β; maxit = 60)
