...
opts = Optim.Options(iterations=30,store_trace = true,
                             show_trace = false,
                             show_warnings = true)
        ...
        for (opt_name,opt) in zip(optim_names, optim_list)
            for (dist_name, dist_f) in zip(dist_names, dist_list)
                function wrap_cost(p::Vector)
                    GW_i = cf(νnGrid.parent, jED.GWeiss(νnGrid.parent, μ, p[1:N], p[(N+1):end]))
                    tmp = cf(νnGrid.parent, GLoc_i.parent)
                    return dist_f(tmp .- GW_i)
                end
                println("running: ", cf, " // opt: ", typeof(opt) , " // dist: ", dist_name)
                @timeit to "run $i" result = optimize(wrap_cost, p0, opt, opts)
                push!(fits, result)
...
names,fits = run_tests()

@timeit to "LsqFit" lsq_fit = curve_fit(GW_fit_real, νnGrid.parent, target, p0)
