...
ek_start = if NSites == 4 
        [-0.1447889658624951, 1.7638353989578364, 0.027020899600905066, -1.021485372109998]
    ...
    elseif NSites == 3
        [0.0291,  -0.1289, 0.6445]
    end
vk_start = if NSites == 4 
        [0.14233737909597746, 0.33360018816844883, 0.10927625405585563, 0.35232028939083465]
...
#cf=transf_01(x, y)
dist_name= "|vec|"#"|vec|^2"
dist_f=jED.abs_dist #jED.square_dist
optim_name ="ConjugateGradient"    ##"BFGS"
opt=ConjugateGradient()   ##BFGS()
...
p, νnGrid, GImp, ΣImp, μ, dens, D = DMFT_Loop(U, μ, β, 1.0, p, maxit = 5000);
