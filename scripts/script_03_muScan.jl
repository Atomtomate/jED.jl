...
function DMFT_Loop(U::Float64, μ_in::Float64, β::Float64, p; maxit = 20)
    Nν  = 2000
    Nk  = 200
    α   = 0.4
    GImp_i = nothing
...
function run_scan()
    ...
    μr = LinRange(0.7*μin,1.3*μin,61) 
    # Ur = from0 ? LinRange(0,6,26)[7:20] : reverse(LinRange(0,6,26)[7:20])
    for μi in μr
        p, νnGrid, GImp, ΣImp, μ, dens, D = DMFT_Loop(U, μi, β, p, maxit = 250);
