...
function DMFT_Loop(U::Float64, μ_in::Float64, β::Float64, dens_fix::Float64; maxit = 20)
    ϵₖ = [0.0291,  0.2296, -0.1289, -0.4249]#,  0.6445,  -0.887]
    Vₖ = [0.153, 0.1794,  0.1985,  0.2763]#,  0.1635,  0.2902]
    p  = AIMParams(ϵₖ, Vₖ)
    ...
    for i in 1:maxit
        ...
        println("Solution using Lsq:  U0NS4  ϵₖ = $(lpad.(round.(p.ϵₖ,digits=4),9)...)")
        println("                     U0NS4  Vₖ = $(lpad.(round.(p.Vₖ,digits=4),9)...)")
