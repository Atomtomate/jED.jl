# ==================================================================================================== #
#                                        GreensFunctions.jl                                            #
# ---------------------------------------------------------------------------------------------------- #
#   Author          : Julian Stobbe                                                                    #
# ----------------------------------------- Description ---------------------------------------------- #
#   Measurement of Green's functions from Eigenspace.                                                  #
# -------------------------------------------- TODO -------------------------------------------------- #
#   Only a first draft, needs refactor and testing.                                                    #
#   Do not hardcode spin up GF!!!                                                                      #
# ==================================================================================================== #


# =========================================== Overlap Matrix =========================================

# ============================================ 1 Particle GF =========================================

function overlap_EDiff(basis::Basis, es::Eigenspace, op::Operator, block_overlaps::Vector{Int}, β::Float64, ϵ_cut::Float64)
    bl = basis.blocklist
    res_EDiff   = Stack{Float64}()
    res_factor  = Stack{Float64}()
    expE        = exp.(-β .* es.evals)
    #TODO: exclude underflow here already
    for (block_from, block_to) in enumerate(block_overlaps)
        if block_to > 0
            slice_from = _block_slice(bl[block_from])
            slice_to   = _block_slice(bl[block_to])
            block_from_start = bl[block_from][1] - 1
            block_to_start = bl[block_to][1] - 1
            ov         = _overlap_list(basis, block_from, block_to, op)
            for (i_from,ev_from) in enumerate(es.evecs[slice_from])
                tmp = similar(ev_from)
                for (i_to,ev_to) in enumerate(es.evecs[slice_to])
                    el_i = (expE[block_from_start + i_from] + expE[block_to_start + i_to])*_overlap_cdagger_ev!(tmp, ev_from, ev_to, ov)^2
                    if abs(el_i) > ϵ_cut
                        push!(res_factor, el_i)
                        push!(res_EDiff, es.evals[slice_from][i_from] - es.evals[slice_to][i_to])
                    end
                end
            end
        end
    end
    return collect(res_factor), collect(res_EDiff)
end

"""
calc_GF_1(es::Eigenspace, freq::ComplexF64, β::Float64[, ϵ_cut = 1e-16])

Computes ``|\\langle i | c^\\dagger | j \\rangle|^2 \\frac{e^{-\\beta E_i} + e^{-\\beta E_j}}{Z (E_j - E_i + freq)}``.
TODO: not tested
"""
function calc_GF_1(basis::Basis, es::Eigenspace, νnGrid::AbstractVector{ComplexF64}, β::Float64; ϵ_cut::Float64=1e-16)
    global to
    Z = calc_Z(es, β)
    res = similar(νnGrid)
    fill!(res, 0.0)
    state = 1
    op =  create_op(basis, state)
    @timeit to "ov" ov = _find_cdag_overlap_blocks(basis.blocklist, op)
    @timeit to "pf2" pf, nf = jED.overlap_EDiff(basis, es, op, ov, β, ϵ_cut)
    @timeit to "for" for νi in eachindex(νnGrid)
        νn = νnGrid[νi]
        for j in 1:length(pf)
            #TODO: reduce memory allocation overhead
            res[νi] -= conj(sum(pf[j] / (νn - nf[j]))) / Z
        end
    end
    return res 
end
