...
function calc_GF_1(
    ...
    !(FPT === eltype(νnGrid) || Complex{FPT} === eltype(νnGrid)) &&
        @warn "Element type of Eigenspace ($FPT) does not match element type of ν-Grid ($(eltype(νnGrid)))! Expect loss of precision."
...
function lehmann_full(
    ...
    return collect(res_ov),
    collect(res_from),
    collect(res_to),
    collect(res_ind_from),
    collect(res_ind_to)
end
